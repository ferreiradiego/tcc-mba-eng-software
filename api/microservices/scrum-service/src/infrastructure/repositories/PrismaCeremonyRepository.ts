...
export class PrismaCeremonyRepository implements CeremonyRepository {
  ...
  async create(ceremony: Omit<Ceremony, 'id' | 'createdAt' | 'updatedAt'>): Promise<Ceremony> {
    return (await prisma.ceremony.create({ data: ceremony })) as any;
  }
  async update(id: string, ceremony: Partial<Omit<Ceremony, 'id' | 'createdAt' | 'updatedAt'>>): Promise<Ceremony | null> {
    return (await prisma.ceremony.update({ where: { id }, data: ceremony })) as any;
  }
