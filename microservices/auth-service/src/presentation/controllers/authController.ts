...
import { generateToken, generateRefreshToken } from '../../infrastructure/auth/jwt';
...
export async function registerController(req: Request, res: Response) {
  ...
  const user = await userRepository.create({ name, email, passwordHash, role: role || 'USER' });
...
export async function refreshController(req: Request, res: Response) {
  ...
  try {
    const payload = require('jsonwebtoken').verify(refreshToken, process.env.JWT_SECRET || 'default_secret');
